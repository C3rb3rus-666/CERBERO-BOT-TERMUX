...
let commandQueueTail = Promise.resolve();
...
export function enqueueGlobalCommandTask(task, meta = {}) {
  ...
  const run = commandQueueTail
    .catch(() => {})
    .then(async () => {
      const dynamicDelayMs = computeDynamicDelayMs(chatId);
      if (dynamicDelayMs > 0) {
  ...
  console.log(`[CMD-QUEUE] enqueued task=${taskId} cmd=${command} chat=${chatId} pending=${pendingCommands} groupActivity=${groupActivity}`);
